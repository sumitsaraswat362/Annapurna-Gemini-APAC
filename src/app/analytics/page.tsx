...
const esgMetrics = [
  { title: "Total Food Saved (Kg)", value: "24,500", icon: <Leaf className="w-6 h-6 text-emerald-400" />, trend: "+12%", desc: "vs last month" },
  { title: "Economic Value Recovered for Farmers (₹)", value: "1.24M", icon: <IndianRupee className="w-6 h-6 text-blue-400" />, trend: "+8%", desc: "in direct benefits" },
  { title: "CO2 Emissions Prevented", value: "4,200 Kg", icon: <CloudOff className="w-6 h-6 text-purple-400" />, trend: "+15%", desc: "carbon offset" },
];
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {esgMetrics.map((metric, idx) => (
            <motion.div
              ...
              className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl relative overflow-hidden group hover:bg-white/10 transition-colors cursor-default"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-24rem)] min-h-[600px]">
          ...
          {/* Chat Section */}
          <div className="lg:col-span-1 flex flex-col h-full bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
              ...
              {messages.length === 0 && (
                <div className="text-center text-gray-400 mt-10">
                  ...
                  <div className="mt-4 flex flex-col gap-2">
                    <button onClick={() => setQuery("Which trucks spoiled this week?")} className="text-xs bg-white/5 hover:bg-white/10 px-3 py-2 rounded-lg text-left transition-colors">
                      "Which trucks spoiled this week?"
                    </button>
                    <button onClick={() => setQuery("Show me the coldest trucks")} className="text-xs bg-white/5 hover:bg-white/10 px-3 py-2 rounded-lg text-left transition-colors">
            ...
            <div className="p-4 bg-black/40 border-t border-white/10">
              <form onSubmit={handleSubmit} className="relative">
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-4 pr-12 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder:text-gray-500"
                />
                <button
                  type="submit"
                  disabled={isLoading || !query.trim()}
                  className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-blue-500 hover:bg-blue-600 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-lg transition-colors"
                >
          ...
          <div className="lg:col-span-2 space-y-6 flex flex-col h-full overflow-y-auto pr-2 pb-8">
            {messages.filter(m => m.role === 'assistant' && m.data).length > 0 ? (() => {
              ...
              return (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl flex flex-col min-h-[300px]">
                      <h3 className="text-lg font-medium flex items-center gap-2 mb-4 text-white">
                      ...
                      <div className="flex-1 min-h-0">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={lastAssistantMessage.data}>
                            ...
                            <XAxis dataKey="truck_id" stroke="#ffffff60" fontSize={12} />
                        ...
                        {lastAssistantMessage.data.map((row: any, i: number) => (
                          <div key={i} className="bg-white/5 border border-white/5 rounded-xl p-4 hover:bg-white/10 transition-colors">
                            <div className="flex justify-between items-center mb-2">
                              <span className="font-mono text-sm font-bold text-gray-200">{row.truck_id}</span>
                              <span className={`text-xs px-2 py-1 rounded-full ${row.status === 'Spoiled' ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/20 text-emerald-400'}`}>
                                {row.status}
                              </span>
                            </div>
                            <div className="flex justify-between text-sm text-gray-400">
                              <span>Temp: <span className="text-white">{row.max_temp_celsius}°C</span></span>
                              <span>Loc: <span className="text-white">{row.last_location}</span></span>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              <motion.div 
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="flex flex-col items-center justify-center text-center opacity-40 bg-white/5 border border-white/10 rounded-2xl p-12 shrink-0 min-h-[250px] mb-6"
              >
                <Bot className="w-16 h-16 mb-6 text-gray-400" />
            ...
            <motion.div 
              ...
              className="bg-gradient-to-br from-white/5 to-white/10 border border-white/10 rounded-2xl p-6 shadow-2xl backdrop-blur-xl relative overflow-hidden shrink-0 mt-auto"
            >
              <div className="absolute top-0 right-0 p-32 bg-red-500/10 blur-[100px] rounded-full mix-blend-screen pointer-events-none" />
              <div className="relative z-10">
                <h3 className="text-lg font-medium flex items-center gap-2 mb-2 text-white">
