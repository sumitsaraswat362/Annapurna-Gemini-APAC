...
import { DEMO_CARGO } from "./src/data/mock-data.ts";
...
async function wipeAndReset() {
  try {
    ...
    console.log("Pushing a valid, fresh mock cargo to jumpstart the database...");
    await setDoc(doc(db, "cargos", DEMO_CARGO.id), { ...DEMO_CARGO, status: "emergency", createdAt: Date.now() });
